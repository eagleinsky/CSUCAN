...
UNS32 _setODentry( CO_Data* d,
                   ...
                   UNS32 * pExpectedSize,
                   UNS8 checkAccess,
                   UNS8 endianize) /*d,目标索引,目标子索引,源数据,长度,chack,大小端*/
{
  UNS32 szData;
  UNS8 dataType;
  UNS32 errorCode;
  ...
  ptrTable =(*d->scanIndexOD)(wIndex, &errorCode, &Callback); /*查找对象字典条目,返回条目地址*/
  if (errorCode != OD_SUCCESSFUL)
    return errorCode;

  if( ptrTable->bSubCount <= bSubindex ) { /*目标子索引不存在*/
    /* Subindex not found */
    accessDictionaryError(wIndex, bSubindex, 0, *pExpectedSize, OD_NO_SUCH_SUBINDEX);
    return OD_NO_SUCH_SUBINDEX;
  }
	
	/* (checkAccess) 为1 并且 状态只读 */
...
//一切顺利，准备改写对象字典
  dataType = ptrTable->pSubindex[bSubindex].bDataType;  //数据类型
  szData = ptrTable->pSubindex[bSubindex].size;         //数据大小
