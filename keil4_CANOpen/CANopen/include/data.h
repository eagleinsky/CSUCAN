...
struct struct_CO_Data {
	...
	const indextable *objdict;          //对象字典
	s_PDO_status *PDO_status;           //PDO状态
	TIMER_HANDLE *RxPDO_EventTimers;    //
	void (*RxPDO_EventTimers_Handler)(CO_Data*, UNS32);
	const quick_index *firstIndex;      //快速查找，对象字典中各类型初始条目
	const quick_index *lastIndex;       //快速查找，对象字典中各类型初始条目
	const UNS16 *ObjdictSize;           //对象字典条目数
	const UNS8 *iam_a_slave;            //是否从节点，1：是从节点。
	valueRangeTest_t valueRangeTest;    //函数
	...
	s_state_communication CurrentCommunicationState;  //states.h 各模块片选。表示是否启用
	...
	/* General */
	UNS8 toggle;
	CAN_PORT canHandle;	          //CAN号，1或者2
	scanIndexOD_t scanIndexOD;    //函数，查找索引 （switch，case）
